---
title: "Reactome enrichment analysis in RE and RS proteins (white matter)"
author: "Yuting Xie"
date: "2023-09-08"
...
info2$BBA <- paste0(info2$BBA, sep="_", "W")
info2$order <- c(1:nrow(info2))
rownames(info2) <- info2$BBA

setwd("Z:/Project/HBA/data_and_code_20230726/2_RE_RS_pathway_enrichment/proteins_white_matter")
ts <- data.table::fread("HBA_ts_9639pro_1060sam_White_BBA_3_2_2.5_20220921.csv",data.table = F)
ts <- data.frame(ts,row.names = "V1")

com_region <- intersect(info2$BBA, rownames(ts))
info3 <- info2[com_region, ]
ts <- ts[info3$BBA, ]
colnames(ts) <- sapply(colnames(ts),function(x){str_split(x,"[.]")[[1]][1]}) 
...
SP <- read.csv("selected_pathways.csv", header = FALSE)
...
setwd("Z:/Project/HBA/data_and_code_20230726/2_RE_RS_pathway_enrichment/proteins_white_matter")
# write.csv(pathway4, "pavalue_0.01_in_top10_pathway_94BBA_Lobe_265pathway_data_for_plot.csv")
ggsave("pavalue_0.01_in_top10_pathway_94BBA_Lobe_27pathway_theme_bw_pvalue_white_matter.pdf", width=16, height=10)
```
