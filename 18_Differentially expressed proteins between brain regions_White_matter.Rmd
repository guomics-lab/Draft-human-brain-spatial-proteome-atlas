...
setwd("W:/members/xieyuting/HBA/differential analysis of brain regions_20230111")
save(abs_log2_FC,file="W_matter_abs_log2_FC.RData")
...
load(file="Protein_W_matter_abs_log2_FC.RData")
load(file="Protein_W_matter_T_test_adj_P.RData")
...
setwd("D:/1_guomics_xyt/1Project/hba_research_workflow/9_DataAnalysis_20220901/differential analysis of brain regions_20230111")
# save(DP, HP, HP_data, file="W_matter_differentially expressed proteins data_DP_HP_HP_data.RData")
load(file="W_matter_differentially expressed proteins data_DP_HP_HP_data.RData")
...
pheatmap(log_HPdata,
         ...
         show_rownames = T, 
         show_colnames = T, 
         filename = "W_matter_differential_protein_numbers_AbsLog2FC_1.5_98_Regions_log2_number_annotation.pdf",       
         main = "Number of differentially expressed proteins between pairs of brain regions in white matter",
         height = 40,width =44)

# HP_order<-as.data.frame(HP_data)
# HP_order$lobe<-df8$Lobe[match(rownames(HP_data),df8$region)]
# HP_order1<-HP_order[order(HP_order$lobe,decreasing=T),] 
# 
# HP_order2<-HP_order1[,-ncol(HP_order1)]
# HP_order3<-HP_order2[,rownames(HP_order2)]
# 
# annotation_col=data.frame(HP_order1$lobe)
# rownames(annotation_col)<-rownames(HP_order1)
# colnames(annotation_col)<-"Lobe"
# 
# 
